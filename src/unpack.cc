...
//
// Custom 71-bit integer struct to replace __int128.
// Required because ESP32/Xtensa doesn't support __int128,
// and free-text FT8 messages are exactly 71 bits long.
//
struct Int71 {
    uint32_t w[3] = {0, 0, 0}; // w[0]: bits 0-31, w[1]: bits 32-63, w[2]: bits 64-70
    
    void shift_left_or(int bit) {
        w[2] = (w[2] << 1) | (w[1] >> 31);
        w[1] = (w[1] << 1) | (w[0] >> 31);
        w[0] = (w[0] << 1) | bit;
    }
    ...
    int get_bit(int i) const {
        if (i < 32) return (w[0] >> (31 - i)) & 1;
        if (i < 64) return (w[1] >> (63 - i)) & 1;
        return (w[2] >> (71 - i)) & 1;
    }
    
    // Replaces x = x / 42; and x % 42; 
    int divmod42() {
        int rem = 0;
        Int71 q;
        for (int i = 70; i >= 0; i--) {
        ...
        *this = q; // x becomes x / 42
        return rem; // returns x % 42
    }
};
...
// (Used for all calls except the 71-bit free text)
//
long long 
...
unpack_0_0(int a77[])
{
  ...
  // Use the custom Int71 struct since __int128 is not available on ESP32
  Int71 x;
  for(int i = 0; i < 71; i++){
    x.shift_left_or(a77[i]);
  }
