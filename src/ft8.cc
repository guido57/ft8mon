            ...
            double &delta_hz)
{
  ...
  hz0 = std::max( (double) 0.0, hz0 - reduce_extra);
  hz1 = std::min( (double) (arate / 2.0f), hz1 + reduce_extra);
...
go(int npasses)
{
  ...
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  nrate = rate_; // don't reduce for now.
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if(do_reduce && nrate > 0 && nrate < rate_ * 0.75){
    ...
    // delta[0]=1.0;
    // samples_ = reduce_rate(delta,
    //                        min_hz_-3.1-go_extra,
    //                        max_hz_+50-3.1+go_extra,
    //                        rate_, nrate, delta_hz);
    // save_h(samples_.data(), samples_.size(), "h.bin");
  ...
  for(pass_ = 0; pass_ < npasses; pass_++){
    ...
    std::vector<Strength> order; // it will hold all the candidates, sorted strongest-first.
    ...
    // e.g. if coarse_hz_n=4, then we try hz_frac=0, 1.5625, 3.125, 4.6875 Hz.                                                  
...
// search for costas blocks in an MxN time/frequency grid.
// but we know the symbols, so we can use a more accurate strength function.
// hz0 +/- hz_win in hz_inc increments. hz0 should be near 25.
...
// given 200 samples/second, try to decode a message.
// return 2 if it decodes to a brand-new message.
// return 1 if it decodes but we've already seen it,
//   perhaps in a different pass.
// return 0 if we could not decode.
...
one_iter(const std::vector<float> &samples200, int best_off, double hz_for_cb)
{
  if(do_second){
    ...
    // just one signal, at best_off and hz_for_cb.
           ...
           const ffts_t &m79)
{
  ...
  if(decode(ll174, a174, use_osd, comment)){
    ...
    if(do_third == 1){
      ...
      // printf("do_third=1 call search_both_known: best_hz %.2f, best_off %.3f\n", best_hz, best_off);
      // search_both_known(samples200, 200, re79,
      //                   best_hz, best_off,
      //                   best_hz, best_off);
      // printf("do_third=1 search_both_known done: best_hz %.2f, best_off %.3f\n", best_hz, best_off);                  
    ...
    if(do_third == 2){
      // fine-tune offset and hz for better subtraction.
      // printf("do_third=2 call search_both_known: best_hz %.2f, best_off %.3f\n", best_hz, best_off);
      // search_both_known(samples_, rate_, re79,
      //                   best_hz, best_off,
      //                   best_hz, best_off);
      // printf("do_third=2 search_both_known done: best_hz %.2f, best_off %.3f\n", best_hz, best_off);
    }
